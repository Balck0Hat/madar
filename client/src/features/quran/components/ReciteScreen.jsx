...
export default function ReciteScreen({ s, a, onBack, onNext }) {
  const num = useNum();
  const whole = a === "all";
  const range = /^(\d+)-(\d+)$/.exec(String(a)); // من آية إلى آية
  const many = whole || Boolean(range);
  const rec = useRecite();
  ...
  const ayahs = data ? (whole ? data.ayahs : range ? data.ayahs.filter((x) => x.a >= Number(range[1]) && x.a <= Number(range[2])) : data.ayahs.filter((x) => x.a === Number(a))) : [];
  ...
  const start = async () => { setSaved(false); try { await rec.begin(Number(s), whole ? null : range ? { from: Number(range[1]), to: Number(range[2]) } : Number(a)); await mic.start(); } catch { /* الخطأ يُعرض من الخطّاف */ } };
  ...
  useEffect(() => { if (finished && rows.length && rows.some((r) => r.heard) && rows.every((r) => !r.heard || r.ok === r.total) && !saved) decide(true); }, [finished, rec.status]); // eslint-disable-line react-hooks/exhaustive-deps

  const shell = (children) => (
    <div style={{ minHeight: "100vh", background: P.bg, color: P.ink, "--unit-color": P.gold }}>
      <TopBar paper title={<span style={{ fontSize: T.lg }}>{many ? "تسميع سورة" : "تسميع"}</span>} onBack={() => { stop(); rec.close(); onBack(); }} />
  ...
  return shell(
    <>
      <div style={{ display: "flex", alignItems: "center", gap: S.lg }}>
        <div style={{ color: P.muted, fontSize: T.sm, flex: 1 }}>سورة {data.name}{whole ? ` · ${num(ayahs.length)} آية` : range ? ` · من ${num(range[1])} إلى ${num(range[2])}` : ` · آية ${num(a)}`}</div>
      ...
      <div style={{ background: P.card, border: `1px solid ${P.line}`, borderRadius: R.x3, padding: S.x4, margin: `${S.x3}px 0`, maxHeight: "50vh", overflowY: "auto" }}>
        <p className="madar-quran" dir="rtl" style={{ margin: 0, fontSize: many ? "1.45em" : "1.7em", lineHeight: 2.1, textAlign: "justify" }}>
          {ayahs.map((x, i) => (
            <span key={x.a} ref={(el) => { rowRefs.current[x.a] = el; }} style={{ background: many && current === x.a ? alpha(P.gold, 0.08) : "transparent", borderRadius: R.sm }}>
      ...
      <div aria-live="polite" style={{ minHeight: S.x6, color: P.muted, fontSize: T.sm, textAlign: "center" }}>
        {rec.error || mic.error ? <span style={{ color: C.red }}>{rec.error || mic.error}</span>
          ...
          : mic.recording ? `يسمع… ${num(heardWords)} من ${num(total)} كلمة${many && current ? ` · آية ${num(current)}` : ""}`
      ...
      {rec.text && mic.recording && (
        <div style={{ textAlign: "center", marginTop: S.md }}>
          <div style={{ color: P.muted, fontSize: T.xs }}>سمعتُ</div>
          <div className="madar-quran" dir="rtl" style={{ fontSize: "1.35em", lineHeight: 1.9, color: P.ink }}>{rec.text}</div>
        </div>
      )}
      {finished && rows.length > 0 && <ReciteSummary rows={rows} saved={saved} onDecide={decide} />}
      {saved && onNext && <div style={{ marginTop: S.x3 }}><Btn paper onClick={() => { rec.close(); onNext(); }}>{whole ? "السورة التالية" : many ? "أعد التسميع" : "الآية التالية"}</Btn></div>}
    </>,
  );
}
