      ...
      <div style={{ padding: "0 16px" }}>
        <PatternBand id={d.id} color={d.color}>
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div><div style={{ fontWeight: 900, fontSize: T.x3 }}>{d.name}</div><div style={{ color: C.muted, fontSize: T.base }}>{d.desc}</div></div>
          </div>
        </PatternBand>
        <div role="tablist" style={{ display: "flex", gap: 6, margin: "14px 0 10px" }}>
          {RING_NAMES.map((r, i) => (
            <button key={r} type="button" role="tab" aria-selected={ringIdx === i} onClick={() => onRing(i)} style={{ flex: 1, background: ringIdx === i ? alpha(d.color, 0.15) : C.surface, border: `1px solid ${ringIdx === i ? d.color : C.line}`, color: C.text, borderRadius: R.lg, padding: "9px 6px", fontSize: T.sm, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 4 }}>
              {i > 0 && !st.ring1Done && <Lock size={11} />}{r}
            </button>
          ))}
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6, fontSize: T.base }}>
        ...
        {locked && (
          <div style={{ background: C.goldSoft, border: `1px solid ${alpha(C.gold, 0.33)}`, borderRadius: R.xl, padding: "10px 12px", marginTop: 12, fontSize: T.base, display: "flex", gap: 8, alignItems: "center" }}>
