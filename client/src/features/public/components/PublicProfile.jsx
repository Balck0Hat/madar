...
export default function PublicProfile({ handle, onHome }) {
  ...
  return (
    <div className="madar-in" style={{ minHeight: "100vh", padding: "24px 18px 40px", display: "grid", alignContent: "start", gap: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><div style={{ fontWeight: 900, fontSize: T.x4 }}>مدار</div><Btn small full={false} onClick={onHome}>ابدأ رحلتك</Btn></div>
      {loading && <Skeleton lines={5} />}
      {error && <ErrorState message={error.message} onRetry={reload} onBack={onHome} />}
      {p && (
        <>
          <div style={{ textAlign: "center" }}>
            <div style={{ fontSize: T.x5, fontWeight: 900 }}>عجلة {p.name}</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8 }}>
            {[["وحدات", p.stats.units], ["خيوط", p.stats.threads], ["قطاعات", p.stats.sectors], ["سلسلة", p.streak]].map(([l, v]) => <div key={l} style={{ background: C.surface, border: `1px solid ${C.line}`, borderRadius: R.xl, padding: "10px 6px", textAlign: "center" }}><div style={{ fontFamily: MONO, fontWeight: 800, fontSize: T.x3, color: C.gold }}>{num(v)}</div><div style={{ color: C.muted, fontSize: T.xs }}>{l}</div></div>)}
          </div>
          {p.certificate && <Card accent={C.gold}><div style={{ display: "flex", gap: 8, alignItems: "center", fontWeight: 800 }}><Award size={16} color={C.gold} />حائز على شهادة الثقافة العامة</div><div style={{ color: C.muted, fontSize: T.base, marginTop: 4, fontFamily: MONO }}>{p.certificate.code}</div></Card>}
          {p.badges.length > 0 && (
            <Card>
          ...
          <div style={{ color: C.muted, fontSize: T.sm, textAlign: "center", display: "flex", justifyContent: "center", gap: 6, alignItems: "center" }}><Link2 size={12} />الخيوط الذهبية على العجلة تربط مجالين معرفيين.</div>
          <OrbitMark size={56} />
        </>
      )}
    </div>
