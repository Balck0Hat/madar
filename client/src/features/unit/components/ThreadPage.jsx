...
export default function ThreadPage({ thread }) {
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 8, color: P.gold, fontWeight: 800, fontSize: ".82em" }}><Link2 size={15} />خيط إلى {to.domainName}: {to.title}</div>
      {/* المتن بخط النسخ كبقية الدرس، والخيارات بخط الواجهة لأنها أزرار */}
      <div style={{ fontFamily: READ, fontSize: "1.03em", lineHeight: 1.95, margin: "12px 0" }}>{thread.text}</div>
      ...
      {picked && (
        <div className="madar-in" role="status" style={{ marginTop: 12, color: P.muted, fontSize: ".88em", lineHeight: 1.7 }}>
