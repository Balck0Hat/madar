...
import { C, MONO, T, S, GUTTER } from "../../../shared/constants/theme";
...
export default function VerifyPage({ code, onHome }) {
  ...
  return (
    <div className="madar-in" style={{ minHeight: "100vh", padding: `${S.x8}px ${GUTTER}px`, display: "grid", alignContent: "start", gap: S.x4 }}>
      <OrbitMark size={72} />
      <div style={{ textAlign: "center", fontWeight: 700, fontSize: T.x4 }}>التحقق من شهادة إتمام</div>
      <div style={{ textAlign: "center", fontFamily: MONO, color: C.muted }}>{code}</div>
      {loading && <Skeleton lines={3} />}
      {error && <ErrorState message={error.message} onRetry={reload} />}
      {data && (
        <Card accent={valid ? C.green : C.red}>
          <div style={{ display: "flex", alignItems: "center", gap: S.xl, fontWeight: 700, fontSize: T.x2, color: valid ? C.green : C.red }}>
            {valid ? <ShieldCheck size={22} /> : <ShieldX size={22} />}{valid ? "شهادة إتمام مسجّلة لدى مدار" : "لا توجد شهادة بهذا الرمز"}
          </div>
          {valid && (
            <div style={{ marginTop: S.xl, lineHeight: 1.9, fontSize: T.lg }}>
              ...
              <div style={{ color: C.muted, fontSize: T.base, marginTop: S.lg, lineHeight: 1.8 }}>
