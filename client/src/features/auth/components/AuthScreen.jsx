...
function Field({ id, label, error, ...props }) {
  return (
    <div>
      <label htmlFor={id} style={{ display: "block", fontSize: 13, fontWeight: 700, marginBottom: 6 }}>{label}</label>
      ...
      {error && <div id={`${id}-err`} style={{ color: C.red, fontSize: 12, marginTop: 6 }}>{error}</div>}
    </div>
  );
}
