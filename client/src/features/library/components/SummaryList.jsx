        ...
        return (
          <Card key={s.unitId} accent={info.color} onClick={() => onOpenUnit(s.unitId)}>
            ...
            <div style={{ display: "grid", gap: 6, marginTop: 10 }}>
              {s.summary.map((line, i) => <div key={i} style={{ display: "flex", gap: 8, fontSize: T.md, lineHeight: 1.7, color: C.text }}><Check size={14} color={C.gold} style={{ flexShrink: 0, marginTop: 5 }} />{line}</div>)}
            </div>
