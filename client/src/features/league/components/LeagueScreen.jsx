      ...
      {data && (
        <>
          <div style={{ display: "flex", gap: 6, overflowX: "auto", padding: "4px 16px 12px" }}>
            {data.tiers.map((t, i) => <span key={t} style={{ flexShrink: 0, fontSize: 11, fontWeight: 700, padding: "5px 10px", borderRadius: 999, background: i === data.tier ? C.goldSoft : C.surface, color: i === data.tier ? C.gold : C.muted, border: `1px solid ${i === data.tier ? C.gold : C.line}` }}>{t}</span>)}
          </div>
          {data.lastLeague && data.lastLeague.outcome !== "stay" && (
            <div style={{ padding: "0 16px 10px" }}>
              <Card accent={data.lastLeague.outcome === "up" ? C.green : C.red}>
                ...
                <div style={{ color: C.muted, fontSize: 13, marginTop: 4 }}>ترتيبك كان {num(data.lastLeague.rank)} في دوري {data.tiers[data.lastLeague.tier]}.</div>
              </Card>
            </div>
          )}
          <div style={{ padding: "0 16px", color: C.muted, fontSize: 13, marginBottom: 10 }}>
