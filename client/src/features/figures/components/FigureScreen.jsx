...
import { getFigure, listFigures } from "../services/figures.service";
...
// معه لأن الشريط الزمني و«المعاصرون» و«المرتبطون» و«التالي» كلها منها.
export default function FigureScreen({ figureId, onBack, onOpen }) {
  const num = useNum();
  ...
  return shell(
    <>
      <FigureHeader figure={f} list={list} color={color} />
      <div role="tablist" style={{ display: "flex", gap: S.lg, margin: `${S.x4}px 0 ${S.x3}px` }}>
      ...
      {depth === "quick" ? (
        ...
        <div key="story" className="madar-in">
          <FigureStory figure={f} list={list} color={color} onOpen={open} onExit={() => setDepth("quick")} />
        </div>
      )}
    </>,
  );
