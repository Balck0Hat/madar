    ...
    <div>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 12 }}>
        {items.map((it, k) => {
          ...
          return (
            <button key={k} type="button" onClick={() => !locked && pos === -1 && onChange([...chosen, k])} style={{ background: pos >= 0 ? alpha(color, 0.2) : C.surface, border: `1px solid ${pos >= 0 ? color : C.line}`, borderRadius: 12, padding: "10px 14px", color: C.text, cursor: "pointer", fontSize: 15, display: "flex", gap: 8, alignItems: "center" }}>
