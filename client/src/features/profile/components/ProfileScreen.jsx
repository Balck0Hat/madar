      ...
      <div style={{ padding: "0 16px", display: "grid", gap: 12 }}>
        <Card>
          <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
            <div style={{ width: 56, height: 56, borderRadius: R.pill, background: C.gold, color: "var(--bg)", display: "grid", placeItems: "center", fontWeight: 900, fontSize: T.x5 }}>{profile.name[0]}</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 900, fontSize: T.x2 }}>{profile.name}</div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: T.sm, color: C.muted, margin: "14px 0 6px" }}><span>إلى المستوى {num(level + 1)}</span><span style={{ fontFamily: MONO }}>{num(cur)}/{num(need)}</span></div>
          <Bar value={cur / need} />
        </Card>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 6 }}>
          {tiles.map(([l, v]) => (
            <div key={l} style={{ background: C.surface, border: `1px solid ${C.line}`, borderRadius: R.xl, padding: "10px 4px", textAlign: "center" }}>
              <div style={{ fontFamily: MONO, fontWeight: 800, fontSize: T.x2, color: C.gold }}>{num(v)}</div>
