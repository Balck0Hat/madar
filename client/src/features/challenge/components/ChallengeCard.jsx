...
export default function ChallengeCard({ onToast }) {
  ...
  if (!ch.question) return <Card><Head /><div style={{ color: C.muted, fontSize: 13, marginTop: 6, lineHeight: 1.7 }}>لا تحدي اليوم، عد غداً.</div></Card>;
  ...
  if (ch.answered && !ch.result) {
    return (
      <Card accent={verdict}>
        ...
        <div style={{ fontWeight: 700, marginTop: 8, fontSize: 14, color: verdict }}>{ch.correct ? "أجبت اليوم إجابة صحيحة" : "أجبت اليوم، والإجابة لم تكن صحيحة"}</div>
        <div style={{ color: C.muted, fontSize: 13, marginTop: 4, fontFamily: MONO }}>سلسلة التحدي: {num(ch.streak)} يوماً</div>
      </Card>
    );
  }
  ...
  return (
    <Card accent={ch.result ? verdict : C.gold}>
      ...
      <div style={{ fontSize: 15, fontWeight: 700, marginTop: 8, lineHeight: 1.6 }}>{ch.question.q}</div>
      <ChallengeQuestion question={ch.question} result={ch.result} submitting={ch.submitting} onSubmit={send} />
      {ch.submitError && <div role="alert" style={{ marginTop: 8, fontSize: 13, color: C.red }}>{ch.submitError.message}</div>}
      {ch.result && (
        <div className="madar-in" role="status" style={{ marginTop: 10, background: alpha(verdict, 0.12), border: `1px solid ${alpha(verdict, 0.4)}`, borderRadius: 12, padding: "10px 12px" }}>
          <div style={{ fontWeight: 800, color: verdict, fontSize: 13 }}>{ch.correct ? "صحيح" : "ليست الإجابة"}</div>
          {ch.result.why && <div style={{ fontSize: 13, lineHeight: 1.7, marginTop: 4 }}>{ch.result.why}</div>}
          <div style={{ color: C.muted, fontSize: 12, marginTop: 6, fontFamily: MONO }}>سلسلة التحدي: {num(ch.streak)} يوماً</div>
        </div>
      )}
    </Card>
  );
