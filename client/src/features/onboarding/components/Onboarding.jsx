...
const TITLES = ["ما اسمك؟", "كم دقيقة يومياً؟", "ما الذي يشدّك أكثر؟"];
const SUBS = ["سيظهر على شهادتك وفي الدوري.", "الوتيرة تحدد موعد إنجازك، ويمكنك تغييرها لاحقاً.", "سنبدأ خريطتك من هنا بعد وحدات المركز."];
...
export default function Onboarding({ onDone }) {
  ...
  const finish = () => onDone({ name: name.trim(), minutes, fav });
  return (
    <div className="madar-in" style={{ minHeight: "100vh", padding: "28px 22px 32px", display: "flex", flexDirection: "column" }}>
      <div style={{ display: "flex", gap: 6, marginBottom: 28 }}>{[0, 1, 2].map((i) => <div key={i} style={{ flex: 1, height: 4, borderRadius: 99, background: i <= step ? C.gold : C.line }} />)}</div>
      <div style={{ fontSize: 26, fontWeight: 800 }}>{TITLES[step]}</div>
      <div style={{ color: C.muted, marginTop: 6, marginBottom: 22 }}>{SUBS[step]}</div>
      {step === 0 && <input aria-label="اسمك" value={name} onChange={(e) => setName(e.target.value)} placeholder="اسمك" style={inputStyle} autoFocus />}
      {step === 1 && (
      ...
      <div style={{ marginTop: 24 }}>
        <Btn primary disabled={step === 0 && !name.trim()} onClick={() => (step < 2 ? setStep(step + 1) : finish())}>{step < 2 ? "التالي" : "افتح الخريطة"}</Btn>
      </div>
    </div>
  );
}
