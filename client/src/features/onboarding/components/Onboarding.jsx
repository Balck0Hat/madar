...
export default function Onboarding({ name, onDone }) {
  ...
  return (
    <div className="madar-in" style={{ minHeight: "100vh", padding: "28px 22px 32px", display: "flex", flexDirection: "column" }}>
      <div style={{ display: "flex", gap: 6, marginBottom: 28 }}>{[0, 1].map((i) => <div key={i} style={{ flex: 1, height: 4, borderRadius: R.pill, background: i <= step ? C.gold : C.line }} />)}</div>
      {step === 0 && <div style={{ color: C.gold, fontWeight: 700, marginBottom: 6 }}>أهلاً {name}</div>}
      <div style={{ fontSize: T.x5, fontWeight: 800 }}>{TITLES[step]}</div>
          ...
          {PACES.map(([m, l, d]) => (
            <Card key={m} onClick={() => setMinutes(m)} accent={minutes === m ? C.gold : null} style={{ borderColor: minutes === m ? C.gold : C.line }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div><div style={{ fontWeight: 800 }}>{l}</div><div style={{ color: C.muted, fontSize: T.base }}>{d}</div></div>
                <div style={{ fontFamily: MONO, fontWeight: 700, color: C.gold, fontSize: T.x2 }}>{m} د</div>
              </div>
            </Card>
          ))}
        </div>
      ...
      {step === 1 && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          {DOMAINS.map((d) => (
            <button key={d.id} type="button" onClick={() => setFav(d.id)} aria-pressed={fav === d.id} style={{ background: fav === d.id ? alpha(d.color, 0.15) : C.surface, border: `1px solid ${fav === d.id ? d.color : C.line}`, borderRadius: R.x2, padding: "14px 12px", color: C.text, textAlign: "start", cursor: "pointer" }}>
              <Icon id={d.id} size={20} color={d.color} />
              <div style={{ fontWeight: 800, marginTop: 8 }}>{d.name}</div>
              <div style={{ color: C.muted, fontSize: T.xs, marginTop: 2, lineHeight: 1.5 }}>{d.desc}</div>
            </button>
          ))}
        </div>
      )}
