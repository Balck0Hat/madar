...
export default function ImportReport({ result }) {
  ...
  return (
    <div role="status" aria-live="polite" style={{ display: "grid", gap: 8 }}>
      <div style={{ background: alpha(tone, 0.12), border: `1px solid ${alpha(tone, 0.4)}`, borderRadius: R.lg, padding: 10, fontSize: T.base, fontWeight: 700, color: tone }}>
        {headline}
      </div>
      <div style={{ display: "grid", gap: 6, maxHeight: 320, overflowY: "auto" }}>
        {reports.map((r, i) => (
          <div key={`${r.unitId || "?"}-${i}`} style={{ background: C.surface2, border: `1px solid ${r.ok ? C.line : alpha(C.red, 0.4)}`, borderRadius: R.lg, padding: 10 }}>
            ...
            {!r.ok && (
              <ul style={{ margin: "6px 0 0", paddingInlineStart: 18, color: C.muted, fontSize: T.sm, lineHeight: 1.9 }}>
