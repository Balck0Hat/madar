...
const Label = ({ children }) => <div style={{ fontSize: T.sm, color: C.muted, fontWeight: 600, marginBottom: S.sm }}>{children}</div>;
...
export default function QuestionEditor({ q, index, onChange, onRemove }) {
  ...
  return (
    <div style={{ background: C.surface, border: `1px solid ${C.line}`, borderRadius: R.xl, padding: S.x2, display: "grid", gap: S.xl }}>
      <div style={{ display: "flex", alignItems: "center", gap: S.lg }}>
        <span style={{ fontWeight: 700, color: C.gold }}>س{index + 1}</span>
      ...
      {q.t === "mcq" && (
        <div style={{ display: "grid", gap: S.md }}>
          ...
          {(q.opts || []).map((o, k) => (
            <div key={k} style={{ display: "flex", gap: S.lg, alignItems: "center" }}>
