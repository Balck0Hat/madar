  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
        {tiles.map(([l, v]) => <div key={l} style={{ background: C.surface, border: `1px solid ${C.line}`, borderRadius: R.xl, padding: "10px 8px", textAlign: "center" }}><div style={{ fontFamily: MONO, fontWeight: 800, fontSize: T.x3, color: C.gold }}>{num(v)}</div><div style={{ color: C.muted, fontSize: T.xs }}>{l}</div></div>)}
      </div>
      <Card>
        ...
        <div style={{ color: C.muted, fontSize: T.sm, marginTop: 8 }}>تُضبط من ملف .env على الخادم. مسودات: {num(data.drafts)}.</div>
      </Card>
      <FunnelPanel />
      <Card>
        <div style={{ fontWeight: 800, marginBottom: 4 }}>أسئلة يُخطئ فيها المتعلمون</div>
        <div style={{ color: C.muted, fontSize: T.sm, marginBottom: 10 }}>مرتبة بعدد الأخطاء؛ الأسئلة المعروضة 3 مرات على الأقل.</div>
        ...
        <div style={{ display: "grid", gap: 10 }}>
          {data.hardQuestions.map((h) => (
            <div key={`${h.unitId}:${h.qid}`}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: T.base, gap: 8 }}>
