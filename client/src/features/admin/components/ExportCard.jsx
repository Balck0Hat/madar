...
import { C, inputStyle, T, S } from "../../../shared/constants/theme";
...
export default function ExportCard({ onToast }) {
  ...
  return (
    <Card>
      <div style={{ fontWeight: 700, marginBottom: S.sm }}>تصدير</div>
      ...
      <div style={{ display: "grid", gap: S.lg }}>
        <Btn disabled={busy} onClick={() => run(exportAll, `madar-units-${stamp()}.json`, `نُزّلت ${data.length} وحدة`)}>
          <span style={{ display: "inline-flex", gap: S.lg, alignItems: "center" }}><Download size={16} />تنزيل كل الوحدات ({data.length})</span>
        </Btn>
