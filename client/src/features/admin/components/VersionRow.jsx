...
export default function VersionRow({ version, current, busy, onRestore }) {
  ...
  return (
    <div style={{ background: C.surface2, border: `1px solid ${confirming ? alpha(C.gold, 0.5) : C.line}`, borderRadius: R.xl, padding: 12, display: "grid", gap: 8 }}>
      ...
      <div style={{ fontSize: T.sm, display: "flex", gap: 12, flexWrap: "wrap" }}>
      ...
      {confirming && (
        <div role="group" aria-label={`تأكيد استعادة النسخة ${version.version}`} style={{ display: "grid", gap: 8, background: alpha(C.gold, 0.1), border: `1px solid ${alpha(C.gold, 0.35)}`, borderRadius: R.lg, padding: 10 }}>
