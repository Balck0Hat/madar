...
export default function PrimaryCard({ info, resuming = false, minutes, eta, onOpen }) {
  ...
  if (!info) {
    return (
      <Card>
        <div style={{ fontWeight: 700 }}>أنهيت المدار الأول</div>
  ...
  return (
    <Card accent={info.color}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: S.lg }}>
        ...
        <span style={{ color: C.muted, fontSize: T.sm, display: "flex", alignItems: "center", gap: S.sm }}>
          <Clock size={12} aria-hidden="true" />{num(info.minutes)} د · {num(xp)} XP
        </span>
      </div>

      <div style={{ fontWeight: 700, fontSize: T.x2, margin: `${S.xl}px 0 ${S.sm}px`, lineHeight: 1.5 }}>{info.title}</div>
      ...
      <div style={{ color: C.muted, fontSize: T.base, marginBottom: S.x2, lineHeight: 1.7 }}>
      ...
      <span className="madar-press" style={{ display: "block" }}>
        <Btn primary onClick={onOpen}>
          <span style={{ display: "inline-flex", alignItems: "center", gap: S.lg }}><Ico size={16} aria-hidden="true" />{label}</span>
        </Btn>
      </span>

      {/* وعد «تعلّم بوتيرتك» يعيش هنا هادئاً، بعيداً عن أي عدّاد ضغط */}
      {eta && (
        <div style={{ color: C.muted, fontSize: T.sm, marginTop: S.xl, textAlign: "center", lineHeight: 1.7 }}>
