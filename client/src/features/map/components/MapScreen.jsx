...
import PoliticsEntry from "./PoliticsEntry";
...
export default function MapScreen({
  ...
  calm: calmProp, onOpenDomain, onOpenUnit, onProfile, onReview, onToast, onFigures, onPolitics, onQuran, onBooks, threadsNew,
}) {
  ...
  return (
    <div className="madar-in madar-tabpad">
      ...
      <div className="madar-wide">
        <div>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: `${S.x4}px ${S.x4}px 0` }}>
            <div style={{ fontSize: T.x4, fontWeight: 700 }}>مدار</div>
            <button type="button" className="madar-press" onClick={onProfile} style={{ background: C.surface, border: `1px solid ${C.line}`, borderRadius: R.pill, padding: `${S.md}px ${S.x2}px ${S.md}px ${S.md}px`, display: "flex", alignItems: "center", gap: S.lg, color: C.text, cursor: "pointer" }}>
              <span style={{ width: 28, height: 28, borderRadius: R.pill, background: C.gold, color: "var(--bg)", display: "grid", placeItems: "center", fontWeight: 700 }}>{profile.name[0]}</span>
              <span style={{ fontSize: T.base, fontWeight: 600 }}>{profile.name}</span>
          ...
          <div style={{ padding: `${S.md}px ${S.xl}px 0`, touchAction: "manipulation", maxWidth: 520, marginInline: "auto" }} {...tilt}>
        ...
        <div>
          <div style={{ padding: `${S.x3}px ${S.x4}px 0`, display: "grid", gap: S.xl }}>
