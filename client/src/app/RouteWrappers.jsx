...
import { VerifyPage, PublicProfile, FigureScreen, PoliticsScreen, CountryScreen, SuraScreen, ReciteScreen, ListenScreen, BookScreen, ChapterScreen } from "./lazyScreens";
...
export const BookRoute = ({ nav }) => { const { bookId } = useParams(); return <BookScreen key={bookId} bookId={bookId} onBack={() => nav(paths.books)} onOpenChapter={(n) => nav(paths.chapter(bookId, n))} />; };
export const ChapterRoute = ({ nav }) => { const { bookId, n } = useParams(); return <ChapterScreen key={`${bookId}:${n}`} bookId={bookId} n={Number(n)} onBack={() => nav(paths.book(bookId))} onOpenChapter={(m) => nav(paths.chapter(bookId, m))} />; };
