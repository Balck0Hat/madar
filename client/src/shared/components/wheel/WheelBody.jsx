...
function Sector({ d, di, r, progress, locked, recommended, highlight, onSelect, p }) {
...
export default function WheelBody({ progress, level, recommended, onSelect, onCenter, compact = false, threadsNew = [], sky = true, animate = false, highlight, palette: p = C }) {
  ...
  return (
    <g className={animate ? "madar-seg" : ""}>
      ...
      {DOMAINS.map((d, di) => d.rings.map((_, r) => (
        <Sector key={d.id + r} d={d} di={di} r={r} progress={progress} locked={r > 0 && !s.ring1Done} recommended={recommended} highlight={highlight} onSelect={onSelect} p={p} />
      )))}
      {doneThreads.map(([a, b]) => {
