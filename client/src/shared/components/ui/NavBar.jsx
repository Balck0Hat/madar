  ...
  return (
    <nav aria-label="التنقل الرئيسي" className="madar-hide-lg" style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 430, background: alpha(C.bg, 0.92), backdropFilter: "blur(10px)", borderTop: `1px solid ${C.line}`, display: "flex", zIndex: 20 }}>
      {MOBILE.map(({ k, label, Icon }) => (
        <button key={k} type="button" onClick={() => onGo(k)} aria-current={path === k ? "page" : undefined} style={{ flex: 1, minWidth: 0, background: "transparent", border: "none", padding: "9px 2px 12px", color: path === k ? C.gold : C.muted, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, fontFamily: FONT, fontSize: 10.5, fontWeight: path === k ? 800 : 500 }}>
